...
func TestScanNumber(t *testing.T) {
	...
	// test JsonNumber
	config := NewConfig(JsonNumber, UnicodeSpace, Stream)
	...
	// test FloatNumber32
	config = NewConfig(FloatNumber32, UnicodeSpace, Stream)
	_, n = config.JsonToCbor("10", code)
	_, y = config.CborToJson(code[:n], out)
	...
	// test SmartNumber32
	config = NewConfig(SmartNumber32, UnicodeSpace, Stream)
	...
	// test SmartNumber
	config = NewConfig(SmartNumber, UnicodeSpace, Stream)
	_, n = config.JsonToCbor("10.2", code)
	_, y = config.CborToJson(code[:n], out)
