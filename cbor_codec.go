...
func encodeInt8(item int8, buf []byte) int {
	if item > MaxSmallInt {
		...
		buf[0] = Type1 & byte(-(item + 1)) // -23..-1
		return 1
	}
	buf[0] = Type0 & byte(item) // 0..23
	return 1
}
...
func encodeInt32(item int32, buf []byte) int {
	if item > 32767 {
		...
		buf[0] = Type0 & Info26 // 65536 to 2147483647
		binary.BigEndian.PutUint32(buf[1:], uint32(item))
		return 5

	} else if item < -32768 {
		if item > -65536 {
			buf[0] = Type1 & Info25 // -65535..-32769
			binary.BigEndian.PutUint16(buf[1:], uint16(-(item + 1)))
			return 3
		}
		buf[0] = Type1 & Info26 // -2147483648..-65536
...
func encodeUint64(item uint64, buf []byte) int {
	...
	buf[0] = Type0 & Info27 // 4294967296 to 18446744073709551615
...
func encodeInt64(item int64, buf []byte) int {
	if item > 2147483647 {
		if item < 4294967296 {
			buf[0] = Type0 & Info26 // 2147483647..4294967296
			binary.BigEndian.PutUint32(buf[1:], uint32(item))
			return 5
		}
		buf[0] = Type0 & Info27 // 4294967296..9223372036854775807
		binary.BigEndian.PutUint64(buf[1:], uint64(item))
		return 9

	} else if item < -2147483648 {
		if item > -4294967296 {
			buf[0] = Type1 & Info26 // -4294967295..-2147483649
			binary.BigEndian.PutUint32(buf[1:], uint32(-(item + 1)))
			return 5
		}
		buf[0] = Type1 & Info25 // -9223372036854775808..-4294967296
...
func encodeBytesStart(buf []byte) int {
	buf[0] = Type2 & byte(IndefiniteLength) // indefinite chunks of byte string
	return 1
}
...
func encodeTextStart(buf []byte) int {
	buf[0] = Type3 & byte(IndefiniteLength) // indefinite chunks of text
	return 1
}
...
func encodeArrayStart(buf []byte) int {
	buf[0] = Type4 & byte(IndefiniteLength) // indefinite length array
	return 1
}
...
func encodeMapStart(buf []byte) int {
	buf[0] = Type5 & byte(IndefiniteLength) // indefinite length map
	return 1
}

func encodeBreakStop(buf []byte) int {
	buf[0] = Type7 & byte(ItemBreak) // break stop for indefinite array or map
	return 1
}
...
func init() {
	...
	// 1st-byte 0..23
	for i := 0; i < Info24; i++ {
		cborDecoders[Type0&byte(i)] = decodeType0SmallInt
	}
	// 1st-byte 24..27
	cborDecoders[Type0&Info24] = decodeType0Info24
	cborDecoders[Type0&Info25] = decodeType0Info25
	cborDecoders[Type0&Info26] = decodeType0Info26
	cborDecoders[Type0&Info27] = decodeType0Info27
	// 1st-byte 28..31
	cborDecoders[Type0&28] = makePanic("reserved")
	cborDecoders[Type0&29] = makePanic("reserved")
	cborDecoders[Type0&30] = makePanic("reserved")
	cborDecoders[Type0&IndefiniteLength] = makePanic("indefinite -na-")

	//-- Type1                  (signed integer)
	// 1st-byte 0..23
	for i := 0; i < Info24; i++ {
		cborDecoders[Type1&byte(i)] = decodeType1SmallInt
	}
	// 1st-byte 24..27
	cborDecoders[Type1&Info24] = decodeType1Info24
	cborDecoders[Type1&Info25] = decodeType1Info25
	cborDecoders[Type1&Info26] = decodeType1Info26
	cborDecoders[Type1&Info27] = decodeType1Info27
	// 1st-byte 28..31
	cborDecoders[Type1&28] = makePanic("reserved")
	cborDecoders[Type1&29] = makePanic("reserved")
	cborDecoders[Type1&30] = makePanic("reserved")
	cborDecoders[Type1&IndefiniteLength] = makePanic("indefinite -na-")

	//-- Type2                  (byte string)
	// 1st-byte 0..27
	for i := 0; i < 28; i++ {
		cborDecoders[Type2&byte(i)] = decodeType2
	}
	// 1st-byte 28..31
	cborDecoders[Type2&28] = makePanic("reserved")
	cborDecoders[Type2&29] = makePanic("reserved")
	cborDecoders[Type2&30] = makePanic("reserved")
	cborDecoders[Type2&IndefiniteLength] = decodeType2Indefinite

	//-- Type3                  (string)
	// 1st-byte 0..27
	for i := 0; i < 28; i++ {
		cborDecoders[Type3&byte(i)] = decodeType3
	}
	// 1st-byte 28..31
	cborDecoders[Type3&28] = makePanic("reserved")
	cborDecoders[Type3&29] = makePanic("reserved")
	cborDecoders[Type3&30] = makePanic("reserved")
	cborDecoders[Type3&IndefiniteLength] = decodeType3Indefinite

	//-- Type4                  (array)
	// 1st-byte 0..27
	for i := 0; i < 28; i++ {
		cborDecoders[Type4&byte(i)] = decodeType4
	}
	// 1st-byte 28..31
	cborDecoders[Type4&28] = makePanic("reserved")
	cborDecoders[Type4&29] = makePanic("reserved")
	cborDecoders[Type4&30] = makePanic("reserved")
	cborDecoders[Type4&IndefiniteLength] = decodeType4Indefinite

	//-- Type5                  (map)
	// 1st-byte 0..27
	for i := 0; i < 28; i++ {
		cborDecoders[Type5&byte(i)] = decodeType5
	}
	// 1st-byte 28..31
	cborDecoders[Type5&28] = makePanic("reserved")
	cborDecoders[Type5&29] = makePanic("reserved")
	cborDecoders[Type5&30] = makePanic("reserved")
	cborDecoders[Type5&IndefiniteLength] = decodeType5Indefinite

	//-- Type7                  (simple values / floats / break-stop)
	// 1st-byte 0..19
	for i := 0; i < 19; i++ {
		cborDecoders[Type7&byte(i)] =
			func(buf []byte) (interface{}, int) { return i, 1 }
	}
	// 1st-byte 20..27
	cborDecoders[Type7&SimpleTypeFalse] = decodeFalse
	cborDecoders[Type7&SimpleTypeTrue] = decodeTrue
	cborDecoders[Type7&SimpleTypeNil] = decodeNil
	cborDecoders[Type7&SimpleUndefined] = func(_ []byte) (interface{}, int) {
		return SimpleUndefined, 1
	}
	cborDecoders[Type7&SimpleTypeByte] = decodeSimpleTypeByte
	cborDecoders[Type7&Float16] = decodeFloat16
	cborDecoders[Type7&Float32] = decodeFloat32
	cborDecoders[Type7&Float64] = decodeFloat64
	// 1st-byte 28..31
	cborDecoders[Type7&28] = makePanic("unassigned")
	cborDecoders[Type7&29] = makePanic("unassigned")
	cborDecoders[Type7&30] = makePanic("unassigned")
	cborDecoders[Type7&ItemBreak] = decodeBreakCode
}
